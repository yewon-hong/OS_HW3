...
void * sibling (void * h)
{
    bm_header bm_next = *h[3];
    if (bm_next == NULL) return NULL;
	else if (h[1] == bm_next[1]) return &bm_next;
    // TODO
}
...
{
	// TODO
    int fit_size = fitting(s);
    return mmap(NULL, sizeof(4096), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0)
